...
const BASENAME: &'static str = "day13";
...
fn run(solver: &mut lib::Solution, filename: &str, is_full: bool) -> Result<()> {
        ...
        span!(Level::INFO, "samples").in_scope(|| {
            for suffix in params.sample {
                ...
                if let Err(e) = load(&mut solver, &filename) {
                    ...
                    if let Err(e) = run(&mut solver, &filename, false) {
                        error!(
        ...
        span!(Level::INFO, "full").in_scope(|| {
            for suffix in params.full {
                ...
                if let Err(e) = load(&mut solver, &filename) {
                    ...
                    if let Err(e) = run(&mut solver, &filename, true) {
                        error!(
                            "{}Failed running against input '{}': {:?}",
                            Paint::mask("🎄 "),
                            filename,
